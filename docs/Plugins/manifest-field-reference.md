...
- **Notes:** Plugin agents appear in `/agents` UI. See [Subagents docs](/en/sub-agents) for frontmatter schema.
...
- **Example (inline):** 
  ...
  ```
- **Notes:** Hooks are merged across all configured sources (user, project, plugin, managed). See [Hooks docs](/en/hooks) for event types and handler schemas.
...
- **Example:**
  ...
  ```
- **Notes:** Monitor format: JSON array of `{name, command, description, when}`. Supports variable substitution. See [Plugins reference](/en/plugins-reference#monitors) for full schema.
