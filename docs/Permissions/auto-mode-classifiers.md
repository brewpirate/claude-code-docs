...
These classifiers are internal implementation details, not user-configurable via the rule grammar. To influence auto-mode decisions, use the `autoMode` object in settings (`environment`, `allow`, `soft_deny`—see [Settings/permissions-security.md](../Settings/permissions-security.md)).
