...
- **Default:** unset
- **Description:** Activates coordinator mode — Claude Code's multi-agent orchestration layer. Gated at compile time by `COORDINATOR_MODE` flag. Session resume auto-flips this env var to match the mode the session was created in. See [Coordinator/](/claude-code-docs/agents/overview/).
