...
- **Compound commands**: Rules apply to each subcommand independently (`;`, `&&`, `||`, `|`, `|&`, `&`, newlines). Bash recognizes these separators and checks each part against rules. When you approve a compound command with "Yes, don't ask again", the harness saves separate rules for each sub-command. Up to 5 rules may be generated per approval.
  ...
  > **Example of what this means:** If you approve `Bash(npm run test && git commit -m *)`, Claude saves two separate rules: one for `npm run test` and one for `git commit -m *`. If you later want to revoke the git commit permission, you can remove that specific rule from your settings without affecting the test runner rule. This also means a broadly-written compound approval can create more persistent rules than you expect.
